	...
	Title.TextSize = 18
	Title.Text = "Welcome Delux#6666"..ver
	...
	addcmd('cframefly',{'cfly'},function(args, speaker)
		...
		Con = RunService.Heartbeat:Connect(function()
			...
			local Cache = {}
			for _, v in ipairs(getconnections(Human.StateChanged)) do
				Cache[#Cache + 1] = v
				v:Disable()
			end
			for _, v in ipairs(getconnections(HRP.Changed)) do
				Cache[#Cache + 1] = v
				v:Disable()
			end
			for _, v in ipairs(getconnections(Human.Changed)) do
				Cache[#Cache + 1] = v
				v:Disable()
			end
